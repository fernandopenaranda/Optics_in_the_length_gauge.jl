...
"""
drude_conductivity(p::Drude_presets) = drude_conductivity(p.h, p.dh, p.T, p.τ, 
...
function drude_conductivity(h, dh, T, τ, xbounds, ybounds, evals; rel_tol = 1e-5, abs_tol = 0)
    C = 2π * τ * ħ_ev_s
    integrand(q) = k_in_plane_bindependent_conductivity(h, dh, T, q)
...
"""
function k_in_plane_bindependent_conductivity(h, dhi, T, q)  
